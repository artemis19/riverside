...
func ListenOnInterface(interfaceName string, snapLen int32, serverStream pb.Viz_CollectClient) {
...
func Listen(cmd *cobra.Command, args []string) {
	...
	check_in_response, err := client.CheckIn(context.Background(), host)
	if err != nil {
	...
	for _, device := range interfaces {
		...
		go ListenOnInterface(device, int32(snapLen), serverStream)
	}
